...
void Server::handlerPackage()
{
    ...
    //create package task to create asynchronous  request or get future or promise
    // create pool task with return result
    //проверка на получение результата и обработка пакета с данными отсылка

    if ( type_package == typePackage::COMMAND_READ) {
        int value;
        std::thread t1([&](int &xx){ xx = container.get(data);},std::ref(value));
        t1.join();
        responseClient(QString::number(value));
    }
...
//        qDebug()<<"Writee to container" <<value;
...
void Server::init()
{
    ...
    std::vector<std::string> heading {"timestamp", "Client UID","Type Package","Command","Data"};
...
void Server::package_log(pPackage package)
{
    ...
    std::vector<std::string> heading {timestamp, uid.toStdString(),type.toStdString(),
